...
import { X, Check, Building2, Sparkles, Crown, Plus, ArrowLeft, ArrowRight, ShieldCheck } from 'lucide-react';
...
export function BankAccountModal({
  ...
  useEffect(() => {
    if (account && isOpen) {
      setNameAr(account.nameAr || account.name || (lang === 'ar' ? 'حساب بنك مصر' : 'Bank Account'));
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const finalName = nameAr.trim() || (lang === 'ar' ? 'حساب بنكي' : 'Bank Account');
    const parsedBalance = parseFloat(balanceStr) || 0;
    onSave(account.id, finalName, parsedBalance);
    onClose();
  };

  const quickBankSuggestions = [
    'حساب بنك مصر',
    'حساب البنك الأهلي',
    'حساب CIB',
    'حساب بنك القاهرة',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-slate-900 leading-tight">
                {lang === 'ar' ? 'إدارة الحساب البنكي' : 'Bank Account Settings'}
              </h2>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5">
          {/* Main Edit Form */}
          <form onSubmit={handleSubmit} id="bankAccountForm" className="space-y-4">
            
            {/* 1. اسم الحساب البنكي مع التوضيح والمثال */}
            <div className="bg-blue-50/40 border border-blue-100/90 rounded-2xl p-4 space-y-2.5">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-black text-blue-900">
                  {lang === 'ar' ? 'اسم الحساب البنكي' : 'Bank Account Name'}
                </label>
              ...
              <p className="text-[12px] text-slate-600 font-medium leading-relaxed">
                {lang === 'ar' 
                  ? 'عدل اسم الحساب كما تريد (مثال: حساب بنك مصر، البنك الأهلي، CIB...)'
                  : 'Customize account name as you wish (e.g. Banque Misr, NBE, CIB...)'}
              </p>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={lang === 'ar' ? 'مثال: حساب بنك مصر' : 'e.g. Banque Misr Account'}
                  className="w-full bg-white border border-blue-200 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 focus:outline-hidden focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-slate-400 font-sans"
                  required
                />
              </div>

              {/* اقتراحات سريعة لأسماء البنوك */}
              {lang === 'ar' && (
                <div className="pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {quickBankSuggestions.map((bank) => (
                      <button
                        key={bank}
                        type="button"
                        onClick={() => setNameAr(bank)}
            ...
            <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
              <div>
                ...
                <span className="text-xs font-bold text-slate-700 block mt-0.5">
                  {nameAr || (lang === 'ar' ? 'حساب بنكي' : 'Bank Account')}
                </span>
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="0.00"
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-900 focus:bg-white focus:border-blue-500 focus:outline-hidden transition-colors"
                />
            ...
            <div className="pt-2 flex items-center justify-end gap-2.5">
              ...
              <button
                type="submit"
                className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs rounded-xl shadow-xs cursor-pointer flex items-center gap-1.5 transition-colors active:scale-95"
              >
          ...
          {/* ثانياً: إضافة حساب بنكي آخر */}
          <div className="bg-gradient-to-br from-amber-50/60 via-slate-50 to-blue-50/50 border border-slate-200 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black text-slate-900">
                  {lang === 'ar' ? 'إضافة حساب بنكي آخر' : 'Add Another Bank Account'}
                </span>
              </div>
            ...
            <p className="text-xs text-slate-600 font-medium leading-relaxed mb-3">
              {lang === 'ar'
                ? 'هل تمتلك أكثر من حساب بنكي (مثلاً بنك مصر + CIB + البنك الأهلي) وتريد إدارتها معاً؟'
                : 'Manage multiple bank accounts under one dashboard.'}
            </p>
            ...
            >
              ...
              <span>{lang === 'ar' ? '+ إضافة حساب بنكي آخر' : '+ Add Another Bank Account'}</span>
            </button>
              ...
              <div className="mt-3 bg-white border-2 border-amber-300 rounded-2xl p-4 shadow-md animate-in slide-in-from-top-2 duration-200">
                ...
                <div className="grid grid-cols-2 gap-2.5 my-3">
                  ...
                  <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3 text-center">
                    <span className="inline-block text-[10px] font-black uppercase text-blue-700 bg-blue-100 px-2 py-0.5 rounded-full mb-1">
                      خطة PLUS
                    </span>
                    ...
                    <div className="text-[11px] text-blue-700 font-medium mt-0.5">
                      {lang === 'ar' ? 'إنشاء حتى 3 حسابات بنكية' : 'Up to 3 bank accounts'}
                    </div>
                  </div>
                  ...
                  <div className="bg-amber-50/80 border border-amber-300 rounded-xl p-3 text-center">
                    ...
                    <div className="text-[11px] text-amber-800 font-medium mt-0.5">
                      {lang === 'ar' ? 'إنشاء حتى 10 حسابات بنكية' : 'Up to 10 bank accounts'}
                    </div>
                  </div>
                </div>
