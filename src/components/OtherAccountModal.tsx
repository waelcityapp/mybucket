...
import { X, Check, ArrowLeftRight, Crown, Plus, ShieldCheck } from 'lucide-react';
import { FinancialAccount, Language } from '../types';

interface OtherAccountModalProps {
  isOpen: boolean;
...
export function OtherAccountModal({
  ...
  useEffect(() => {
    if (account && isOpen) {
      setNameAr(account.nameAr || account.name || (lang === 'ar' ? 'وسائل دفع واستلام أخرى' : 'Other Payment Methods'));
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const finalName = nameAr.trim() || (lang === 'ar' ? 'وسائل دفع واستلام أخرى' : 'Other Methods');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-slate-900 leading-tight">
                {lang === 'ar' ? 'إدارة وسائل دفع واستلام أخرى' : 'Other Payment & Receiving Methods'}
              </h2>
              <p className="text-xs text-slate-500 font-medium">
                {lang === 'ar' ? 'تعديل اسم الحساب ورصيده الحالي' : 'Edit method name & current balance'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5">
          {/* Main Edit Form */}
          <form onSubmit={handleSubmit} id="otherAccountForm" className="space-y-4">
            
            {/* 1. اسم الحساب مع التوضيح والمثال */}
            <div className="bg-amber-50/40 border border-amber-100/90 rounded-2xl p-4 space-y-2.5">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-black text-amber-900">
                  {lang === 'ar' ? 'اسم وسيلة الدفع / الاستلام' : 'Method Name'}
                </label>
              ...
              <p className="text-[12px] text-slate-600 font-medium leading-relaxed">
                {lang === 'ar' 
                  ? 'عدل اسم الحساب كما تريد (مثال: باي بال PayPal، ويسترن يونيون، شيكات بنكية، عهدة مالية، جمعية شهرية...)'
                  : 'Customize method name as you wish (e.g. PayPal, Western Union, Checks, Petty Cash, Monthly ROSCA...)'}
              </p>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={lang === 'ar' ? 'مثال: باي بال PayPal' : 'e.g. PayPal'}
                  className="w-full bg-white border border-amber-200 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 focus:outline-hidden focus:border-amber-500 focus:ring-2 focus:ring-amber-100 transition-all placeholder:text-slate-400 font-sans"
                  required
                />
              </div>
            ...
            <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
              <div>
                ...
                <span className="text-xs font-bold text-slate-700 block mt-0.5">
                  {nameAr || (lang === 'ar' ? 'وسائل دفع واستلام أخرى' : 'Other Methods')}
                </span>
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="0.00"
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-lg font-black text-slate-900 focus:bg-white focus:border-amber-500 focus:outline-hidden transition-colors"
                />
            ...
            <div className="pt-2 flex items-center justify-end gap-2.5">
              ...
              <button
                type="submit"
                className="px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-black text-xs rounded-xl shadow-xs cursor-pointer flex items-center gap-1.5 transition-colors active:scale-95"
              >
          ...
          {/* ثانياً: إضافة وسيلة دفع واستلام أخرى */}
          <div className="bg-gradient-to-br from-amber-50/60 via-slate-50 to-orange-50/50 border border-slate-200 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black text-slate-900">
                  {lang === 'ar' ? 'إضافة وسيلة دفع واستلام أخرى' : 'Add Another Method'}
                </span>
              </div>
            ...
            <p className="text-xs text-slate-600 font-medium leading-relaxed mb-3">
              {lang === 'ar'
                ? 'هل تستخدم عدة وسائل دفع أو استلام مختلفة (مثل باي بال + ويسترن يونيون + عهدة مالية) وتريد متابعة كل منها على حدة؟'
                : 'Manage multiple payment and receiving methods like PayPal, Western Union, Petty Cash separately.'}
            </p>
            ...
            >
              ...
              <span>{lang === 'ar' ? '+ إضافة وسيلة دفع واستلام أخرى' : '+ Add Another Method'}</span>
            </button>
              ...
              <div className="mt-3 bg-white border-2 border-amber-300 rounded-2xl p-4 shadow-md animate-in slide-in-from-top-2 duration-200">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div>
                    <h4 className="text-sm font-black text-slate-900 leading-snug">
                      {lang === 'ar' ? 'ترقية الخطة لإضافة وسائل إضافية' : 'Plan Upgrade Required'}
                    </h4>
                    <p className="text-xs font-bold text-amber-900 mt-1 leading-relaxed bg-amber-50 p-2.5 rounded-xl border border-amber-200">
                      {lang === 'ar'
                        ? 'يجب الانتقال إلى الخطة Plus حيث يمكنك إنشاء 3 وسائل أو الخطة Pro حتى 10 وسائل'
                        : 'Upgrade to Plus plan (up to 3 methods) or Pro plan (up to 10 methods).'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2.5 my-3">
                  ...
                  <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3 text-center">
                    <span className="inline-block text-[10px] font-black uppercase text-amber-800 bg-amber-100 px-2 py-0.5 rounded-full mb-1">
                      خطة PLUS
                    </span>
                    ...
                    <div className="text-[11px] text-amber-800 font-medium mt-0.5">
                      {lang === 'ar' ? 'إنشاء حتى 3 وسائل منفصلة' : 'Up to 3 separate methods'}
                    </div>
                  </div>
                  ...
                  <div className="bg-amber-50/90 border border-amber-400 rounded-xl p-3 text-center">
                    ...
                    <div className="text-[11px] text-amber-900 font-medium mt-0.5">
                      {lang === 'ar' ? 'إنشاء حتى 10 وسائل متعددة' : 'Up to 10 multiple methods'}
                    </div>
                  </div>
                </div>
